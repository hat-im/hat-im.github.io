(function () {
    ...
    function clearAllSavedData() {
        ...
        // DOM Cache - populated after DOM creation
        const DOMCache = {
            elements: null,
        ...
        // Popup message pools, loaded from data/messages.json before init()
        let MESSAGES = {};
        ...
        function showFinalStats() {
            const randomMessage = MESSAGES.correct[Math.floor(Math.random() * MESSAGES.correct.length)];
            ...
            buttonsContainer.innerHTML = `<button class="popup-button" onclick="startNewSession()">Play More</button>`;
        ...
        function showPopup(type) {
            ...
            switch (type) {
                ...
                case 'correct':
                    ...
                    break;
                case 'revealed':
                    messages = MESSAGES.revealed;
                    buttonText = "Continue";
                    break;
                case 'restart':
                    messages = MESSAGES.restart;
                    buttonText = "Restart";
                    break;
                default:
                    return;
        ...
        function createGameInterface(shouldRestoreGrid = true) {
            // Clear DOM cache before recreating interface
            DOMCache.clear();
        ...
        function calculateCellPosition(word, letterIndex) {
            if (word.direction === 'across') {
                ...
                return { row: word.row + letterIndex, col: word.col };
            }
        }

        function fillCell(row, col, letter) {
        ...
        function fillWord(word) {
            for (let i = 0; i < word.word.length; i++) {
                const { row, col } = calculateCellPosition(word, i);
                fillCell(row, col, word.word[i]);
            }
        }
        ...
        function revealLetter() {
            ...
            for (let word of crosswordData.words) {
                for (let i = 0; i < word.word.length; i++) {
                    const { row: wordRow, col: wordCol } = calculateCellPosition(word, i);
            ...
            // Restore focus after revealing letter
            if (selectedCell && selectedCell.row !== null && selectedCell.col !== null) {
                setTimeout(() => selectCell(selectedCell.row, selectedCell.col), 0);
            }
        }
        ...
        function revealWord() {
            ...
            // Restore focus after revealing word
            if (selectedCell && selectedCell.row !== null && selectedCell.col !== null) {
                setTimeout(() => selectCell(selectedCell.row, selectedCell.col), 0);
            }
        }
        ...
        function checkPuzzle() {
            ...
            // Restore focus to maintain active state
            if (selectedCell && selectedCell.row !== null && selectedCell.col !== null) {
                ...
                crosswordData.words.forEach(word => {
                    ...
                    for (let i = 0; i < word.word.length; i++) {
                        ...
                        if (wordRow === row && wordCol === col) {
                            ...
                            // Restore focus after checking
                            setTimeout(() => selectCell(row, col), 0);
